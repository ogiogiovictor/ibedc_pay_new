...
class TransactionDetails extends Component
{
    ...
    public function checkPaymentStatus($id) {
       ...
       // if (isset($flutterResponse['status']) && $flutterResponse['status'] == "success" && isset($flutterResponse['data']['status']) && $flutterResponse['data']['status'] == 'successful') {
            ...
            if($this->transactions->provider == 'FCMB') {
                $provider = 'FCMB';
            } else {
                $provider = 'Polaris';
            }
            ...
            $audit_description = "Payment for ". $this->transactions->transaction_id. " is ". $flutterResponse['data']['status'];
            AuditLogService::logAction('Check Payment', $this->user->authority,  $audit_description, $this->user->id, 200);
            ...
            $audit_description = "Payment for ". $this->transactions->transaction_id. " is " ;
            AuditLogService::logAction('Check Payment', $this->user->authority,  $audit_description, $this->user->id, 200);
    ...
    public function checkFCMBPayment($id) {
        ...
        $flutterData = [
            'SECKEY' =>  env("FLUTTER_FCMB_KEY"), // FLWSECK-641d8833b7c2105ad0d38fbf7001cf13-18d7868ed56vt-X || 'FLWSECK-d1c7523a58aad65d4585d47df227ee25-X',
            "txref" => $this->transactions->transaction_id
        ];
       ...
       // if (isset($flutterResponse['status']) && $flutterResponse['status'] == "success" && isset($flutterResponse['data']['status']) && $flutterResponse['data']['status'] == 'successful') {
            ...
            if($this->transactions->provider == '' || $this->transactions->provider == 'NULL'){
                $update = PaymentTransactions::where("transaction_id", $this->transactions->transaction_id)->update([
                    'provider' => 'Polaris'
                ]);
            }
    ...
    public function addProviderReference($id) {
        ...
        }else if($this->transactions->provider === 'Polaris'){

            $update = PaymentTransactions::where("transaction_id", $this->transactions->transaction_id)->update([
                'provider' => 'Polaris'
            ]);

        }else if($this->transactions->provider === ''){
        ...
        $flutterData = [
            'SECKEY' =>  $providerKey, // FLWSECK-641d8833b7c2105ad0d38fbf7001cf13-18d7868ed56vt-X || 'FLWSECK-d1c7523a58aad65d4585d47df227ee25-X',
            "txref" => $this->transactions->transaction_id
        ];
    ...
    public function changeprovider($id, $providerText) {
        ...
        Session::flash('success', 'Provider Successfully Changed.');
    }
