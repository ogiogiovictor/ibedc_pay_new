                      ...
                      <div class="card">
                        <div class="card-body">
                          <div class="d-flex flex-wrap justify-content-between">
                            <h4 class="card-title">All Users Virtual Accounts</h4>
                          ...
                          <div class="table-responsive">
                            <table class="table center-aligned-table">
                              ...
                              @if(count($vaccounts['links']) > 0)

                              @foreach($vaccounts['data'] as $vaccount)

                                    <tr>
                                        <td>{{ \Carbon\Carbon::parse($vaccount['created_at'])->format('Y-m-d H:i:s') }}</td>
                                        <td>{{ $vaccount['user_id'] }}</td>
                                        <td>{{ $vaccount['account_name'] }}</td>
                                        <td>{{ $vaccount['customer_email'] }}</td>
                                        <!-- <td>{{ \App\Models\User::where('id', $vaccount['user_id'])->value('name') }}</td> -->
                                        <td>
                                            <div class="text-dark font-weight-medium btn btn-sm btn-success">
                                                <b>{{  $vaccount['account_no'] }}</b>
                                            </div>
                                        </td>
                                        <td>{{ \App\Models\Wallet\WalletUser::where('id', $vaccount['user_id'])->value('wallet_amount') }}</td>
                                        <td>{{ $vaccount['bank_name'] }}</td>
                                        <td>{{ $vaccount['account_reference'] }}</td>
                                        <td>{{ $vaccount['status'] }}</td>
                                      
                                        <td>
                                            <a href="/view_user_virtual_account/{{ $vaccount['id'] }}/{{ $vaccount['user_id'] }}/{{ $vaccount['customer_email'] }}" class="mr-1 p-2 btn btn-xs btn-primary">View</a>
                                        </td>
                                    </tr>
                                @endforeach
                            @else
                            ...
                            <nav>
                                    <ul class="pagination">
                                        @foreach($vaccounts['links'] as $link)
